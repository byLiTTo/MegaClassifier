...
def consumer_func(q, return_queue, model_file, confidence_threshold):
    ...
    while True:
        ...
        image = r[1]
        if verbose:
            print("De-queued image {}".format(im_file))
            sys.stdout.flush()
...
def run_detector_with_image_queue(
    image_files, model_file, confidence_threshold, quiet=False
):
    """
...
def process_images(
    im_files, detector, confidence_threshold, use_image_queue=False, quiet=False
):
    """
    ...
    if use_image_queue:
        run_detector_with_image_queue(
            im_files, detector, confidence_threshold, quiet=quiet
        )
    else:
        results = []
        for im_file in im_files:
            results.append(
                process_image(im_file, detector, confidence_threshold, quiet=quiet)
            )
        return results
...
def process_image(im_file, detector, confidence_threshold, image=None, quiet=False):
    ...
    try:
        ...
        result = {
            "file": result_aux["file"],
            "max_detection_conf": result_aux["max_detection_conf"],
            "time_inference": elapsed,
            "detections": result_aux["detections"],
        }
    except Exception as e:
        if not quiet:
...
def load_and_run_detector_batch(
    ...
    if n_cores > 1 and use_image_queue:
        print(
            "Warning: multiple cores requested, but the image queue is enabled; parallelization with the image queue is not currently supported, defaulting to one worker"
        )
        n_cores = 1
    ...
    if use_image_queue:
        ...
        results = run_detector_with_image_queue(
            image_file_names, model_file, confidence_threshold, quiet
        )
...
def write_results_to_file(
    results, output_file, relative_path_base=None, detector_file=None
):
    """
...
def main():
    ...
    parser.add_argument(
        "--resume_from_checkpoint", help="Path to a JSON checkpoint file to resume from"
    )
    parser.add_argument(
        "--ncores",
        type=int,
    ...
    if args.output_relative_filenames:
        assert os.path.isdir(
            args.image_file
        ), "Could not find folder {}, must supply a folder when --output_relative_filenames is set".format(
            args.image_file
        )
